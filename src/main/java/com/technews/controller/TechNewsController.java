...
public class TechNewsController {
  ...
  public String login(@ModelAttribute User user, Model model, HttpServletRequest request) throws Exception {
    ...
    // populate sessionUser variable by identifying the email of the user
    ...
    try {
      ...
      // We will catch an error and notify client that email address is not recognized, return to login.html template to try again
    ...
    // create a new session variable, set loggedIn to true, and return the dashboard.html template
  ...
  public String signup(@ModelAttribute User user, Model model, HttpServletRequest request) throws Exception {
    ...
    try {
      // Encrypt password and save new user to the database
  ...
  // add a post from user's dashboard
  @PostMapping("/posts")
  public String addPostDashboardPage(@ModelAttribute Post post, Model model, HttpServletRequest request) {
    ...
    // if session is not valid, redirect user to login.html
    if (request.getSession(false) == null) {
      return "redirect:/login";
    } else {
      // set the userId for the post to be the id of the sessionUser
  ...
  public String createCommentCommentsPage(@ModelAttribute Comment comment, Model model, HttpServletRequest request) {
    ...
    if (comment.getCommentText().isEmpty() || comment.getCommentText().equals(null)) {
      ...
      if (request.getSession(false) != null) {
        // set comment userId to be equal to the current sessionUser id
        User sessionUser = (User) request.getSession().getAttribute("SESSION_USER");
